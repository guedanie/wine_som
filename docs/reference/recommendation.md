...
### Named-retailer requests (2026-07-29)
"Anything from HEB?" used to only **filter the already-fetched pool** (`_detect_retailer` → `retailer_pool`), so when the generic fetch happened to surface no H-E-B wine the filter silently no-opped and Somm reported "nothing from H-E-B" — though H-E-B near 78230 stocks 300+ in-budget reds. There was a targeted fetch for region/store/named-bottle but none for a retailer chain. Fix:
- **`detect_retailer(message, nearby_retailers)`** (`candidate_filters.py`) is data-driven — matched against the retailers actually nearby, so every chain works without hand-maintenance, and it can never return a retailer the user isn't near. Aliasing is **punctuation-insensitive per token** (`_norm_retailer` strips non-alphanumerics), so `heb` / `HEB` / `h-e-b` / `h.e.b` all resolve to "H-E-B" — the alias map is *required* for H-E-B specifically, since its name tokenizes to h/e/b and no fuzzy match can catch it. Multi-word + shorthand + typos handled via `difflib` (cm→Central Market, "centrl market"→Central Market).
- **`_retailer_rows`** (`recommend.py`): a fetch scoped to the named retailer's nearby store IDs (in-stock, in-budget, carrying any resolved region constraints), merged into the pool *before* filtering — so the retailer's inventory is genuinely searched.
- **Filter:** retailer has fits → show only that retailer's wines; no fits → keep alternatives from other shops.
- **Honest wording:** `requested_retailer` + `retailer_has_fit` drive a `_build_user_message` directive — either "every listing is from {R}" or "{R} doesn't stock a wine matching their profile; say so plainly, then offer the closest fits elsewhere" (replaces the ambiguous "nothing turned up in this search"). Acceptance: `scripts/verify_retailer_fetch.py`.
- **Fresh cards on a retailer re-ask:** follow-ups ride `conversational: true`, and the cards-vs-text call is made by Claude via `_FOLLOWUP_CONVERSATIONAL_DIRECTIVE` ("answer conversationally UNLESS the user wants DIFFERENT or ADDITIONAL wines"). "Anything from heb?" is ambiguous enough that Claude read it as a question about the wines already on screen, so no new cards appeared. Rather than a frontend heuristic (`naturalChatMode` is only a global on/off flag — there is no re-ask detector), the has-fit branch of the retailer directive now states it deterministically: *"This IS a request for different wines: return picks from {R}; do NOT answer conversationally with picks: []."* The no-fit branch deliberately does NOT demand picks (there's nothing at that retailer to show, so it stays conversational).

### Axis extraction — phrasing robustness (2026-07-30)
The oracle only counts what the intent names, so a parse that extracts nothing silently bypasses the whole safety net. Production red-teaming found exactly that: `"nothing from Mendoza right?"` parsed to `region=None` → **zero axes** → no facts → the turn graded PASS purely because there was nothing to contradict. Negative/rhetorical framings are where absence-bait questions live, so the net was absent precisely where it was needed. Two layers now:
- **Parser prompt** (`intent.py`): extract the named entity EVEN WHEN the user asserts/questions its absence ("surely you have no Barolo", "I assume there's no rosé"); a retailer name is never a `wine_name`.
- **Deterministic fallback** (`availability.py`): `catalog_terms()` builds a normalized vocabulary from `DISTINCT region, sub_region, country, varietal` + grape arrays (~2,300 terms, cached 1h), and `terms_in_message()` scans the raw message — whole-word, accent-folded, longest-match-first, stopword-guarded. `axes_from_intent(..., fallback_terms=…)` uses it ONLY when the parse produced no content axis, so it is a floor, never an override. **Catalog-derived, not curated:** a curated gazetteer (reference.py APPELLATIONS + REGION_COUNTRY + aliases + KNOWN_GRAPES) measured 146 terms and missed 6 of 11 probe terms (Barolo, Chablis, Sancerre, Pauillac, Brunello di Montalcino, Vinho Verde); the catalog vocabulary missed none and self-updates as scrapers add wines.
- **Per-axis isolation**: a failing axis returns no fact for itself only — previously one transient `httpx.ReadError` collapsed *every* fact, silently voiding the net.
- **Dedupe + scope guard**: `(kind, value, scope)` dedupe kills the duplicate `Champagne` rows and the degenerate `H-E-B at H-E-B` axis.
- **Bounded pagination**: `catalog_terms` stops on a short page and at `_CATALOG_MAX_PAGES` — its exit previously depended on the DB returning an empty page, which spun forever against a client that always returns rows.
- Verified live: `"nothing from Mendoza right?"` → `mendoza: PRESENT_NOT_SHORTLISTED, 401 nearby / 394 in budget` (was: no fact at all). Acceptance in `scripts/verify_availability_oracle.py`.
...
### Availability oracle (2026-07-30)
The shortlist is a bounded, ranked, diversity-capped *sample*, so it can answer "what should I recommend?" but structurally **cannot** answer "what exists nearby?" — Somm answering the second question from the first is what produced the false-absence bug class (audit: `docs/recommendation-architecture-audit.md`, plan item #2). The oracle splits the two questions: absence becomes a deterministic counted fact, and the prompt may only assert absence when a fact says so.
- **Module** `recommendation/availability.py` (pure state logic + a parallel count layer). `axes_from_intent(resolved, scope_label, scope_store_ids)` turns the constraints the user *literally named* (place / grape / type / name, plus a scoped copy per named retailer or store — "Barolo at Geraldine's" yields both the nearby and the Geraldine's fact) into ≤6 queryable axes. Nothing named → no axes → no queries → no added latency.
- **Counting** `fetch_axis_counts` runs the axes in a `ThreadPoolExecutor` against FULL nearby inventory, deliberately bypassing every stage that narrows the shortlist: row limits, budget (counted separately), the staleness window, the enrichment gate and the type gate. Price range (`min_price`/`max_price`) is fetched only when something exists but nothing is in budget. Fails open to `{}` on any error — the oracle never blocks a recommendation.
- **Five states** (`derive_state`, precedence order): `UNMEASURED` → `NOT_IN_CATALOG` (total 0) → `PRESENT_OUT_OF_BUDGET` (in-budget 0) → `PRESENT_SHORTLISTED` / `PRESENT_NOT_SHORTLISTED` (by `count_shortlisted` against the final `top`). **`NOT_IN_CATALOG` is the only state that licenses an absence sentence.**
- **Over-match bias (design property).** The predicates are deliberately BROAD — union over `region`/`sub_region`/`country`/`varietal`/`name`, both jsonb casings for grapes, the `rose`/`rosé` enum drift. Over-counting merely declines to claim absence; under-counting reproduces the exact bug. When in doubt, match more.
- **Wiring** `recommend.py` computes counts early (right after `detect_store`/`detect_retailer`) and finalizes states at prompt-build time inside `event_gen` against the final `top`, writing `resolved["availability_facts"]`.
- **Prompt** `claude_client.py` renders a `[VERIFIED AVAILABILITY]` block (`format_fact_block`, each fact carrying a per-state script) plus three licence rules: absence may be stated *only* for `NOT_IN_CATALOG`; any listing satisfying a literally-named constraint MUST appear in the picks (the "never pad" rule covers unrequested filler, not a constraint match); never contradict a verified fact. The old denial phrase ("nothing matching that turned up nearby") is **deleted** from `_SYSTEM_PROMPT`.
- **Retired** the two ad-hoc flags the facts replace: `named_bottle`/`named_bottle_found` and `requested_retailer`/`retailer_has_fit` (producer in `recommend.py`, consumer directives in `claude_client.py`, and their tests). The `retailer_pool` filter itself is unchanged.
- **Tripwire** `_false_absence_suspects(narrative, facts)` (`recommend.py`) is a pure function: absence language in the finished narrative + any `PRESENT_*` fact → the contradicted facts. Called post-`[DONE]`, in its own try/except alongside session persistence, so it can never break or delay a stream; hits log `FALSE_ABSENCE_SUSPECT` and fire a Slack alert (`SLACK_WEBHOOK_URL`). Tests: `tests/test_availability.py`, `tests/test_false_absence_tripwire.py`. Acceptance against live inventory: `scripts/verify_availability_oracle.py`.

#### Deterministic availability line (2026-07-30)
The oracle computes the truth; two rounds of prompt hardening (red-team PASS 50%→85%, `UNUSED_FACT` 6→0) still left the *model* free to misstate it in 2 of 13 probes (`docs/oracle-verification-round2.md`): **axis substitution** ("any Brunello di Montalcino under $50?" → the narrative counted the broader Montalcino, 12, and never counted the axis the user named, 4) and **sycophantic agreement** ("nothing from Mendoza right?" → *"No Mendoza here"* against 394 in budget). A third prompt round would be pleading with the model about a number we already hold, so the fix applies the oracle's own move one layer out: **we render the fact ourselves.** Spec: `docs/superpowers/specs/2026-07-30-availability-line-design.md`.
- **`availability_lines(facts, top, budget_max) -> List[str]`** (pure, `availability.py`) renders only *informative* facts, capped at 3 lines: `PRESENT_NOT_SHORTLISTED` → `394 mendoza in budget nearby ($4–$73) · none in this list` (or `N more …` when some are shown); `PRESENT_OUT_OF_BUDGET` → `3 Barolo at Geraldine's ($59–$110) · above your $50`; `NOT_IN_CATALOG` → `No X nearby` (the one licensed absence); `UNMEASURED` → `Couldn't confirm X nearby`. `PRESENT_SHORTLISTED` is **silent** — the picks already answer it, so a plain "recommend me a red" never gets a strip. Scoped axes render `at <scope>`, unscoped render `nearby`.
- **Narrow beats broad.** Any axis whose folded value is a substring of another axis's value is dropped, so `Brunello di Montalcino` renders and `Montalcino` / `Brunello` are suppressed. The substitution failure is structurally impossible in the line.
- **Computed remainder.** `shown` comes from `count_shortlisted(axis, top)`, so "none in this list" / "N more" is arithmetic, never the model's `in_budget` (which it mislabelled as the remainder across 4 otherwise-passing probes). `in_budget <= shown` → silent.
- **Transport** `recommend.py`'s `event_gen` emits `{"type":"availability","lines":[…]}` immediately after the `picks` frame — a footnote that never delays first text — only when non-empty, wrapped in try/except so a render failure logs and the stream continues. Each fact dict now carries `"axis": a` (the lines need it for `count_shortlisted` and the narrow-beats-broad guard). Unknown event types are already ignored client-side, so an older client degrades safely.
