...
- **Deferred:** a retailer-named *follow-up* is sent `conversational: true`, so Claude may answer in text rather than spawning fresh cards — that's a frontend re-ask heuristic (`naturalChatMode`), not this fix.
