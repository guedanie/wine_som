...
| **8** | **Multi-turn intent state** — persist resolved intent per session; parse expresses a delta; run `detect_store`/`detect_retailer` over carried intent | Turn 2 stops reverting to a generic pool while the model still remembers the constraint | M | Medium — needs explicit reset semantics or users get stuck in a stale filter | Every fix in #2–#6 applying only to the first turn |
