...
24. **Twin Liquors scraper (PROBED ✅ 2026-07-09 — VIABLE, build next)** — `twinliquors.com` = City Hive, but the anonymous auth that blocked the Nashville City Hive shops is cracked: `api.cityhive.net/api/v1/products/search.json?merchant_id=&api_key=&client_origin=app://sites.twinliquors&text=` returns price + `quantity` (stock) + per-store `merchant_name`/`coordinates`/`full_address`, no login. api_key is in the shop HTML (`window.cityHiveWidgetLoaderConfig.apiKey`). Per-store model (each store = merchant_id, ~90 TX stores — enumerate like Spec's); 30-result cap per term → multi-term search + dedupe by id (Kroger pattern). Full reference: `data/exploration/twinliquors_findings.md`. **Reusable: likely unblocks parked Nashville City Hive shops (Corkdorks, Frugal MacDoogal)**
