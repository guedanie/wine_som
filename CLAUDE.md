...
| Weekly scrape workflow | `.github/workflows/weekly-scrape.yml` | GitHub Actions cron Sunday 02:00 CT — all 11 scrapers (7 SA/Austin + Harvest + Pogo's + Kroger + Twin Liquors) + `--null-only` extraction; Slack notify per-scraper; each step `continue-on-error`. **Twin Liquors (Cloudflare) may 1015 on GitHub datacenter IPs — continue-on-error absorbs it; move to the mini if it proves blocked (like Vivino). Confirm with a `workflow_dispatch` test run.** |
...
24. ~~Twin Liquors scraper~~ ✅ **BUILT + wired into weekly-scrape.yml 2026-07-09** — `backend/scrapers/twin_liquors.py`. Remaining: (a) `workflow_dispatch` test run to confirm GitHub datacenter IPs aren't Cloudflare-1015-blocked (if they are, move this one step to the mini — the scraper's paced+backoff-safe either way); (b) enumerate the full ~90-store merchant_id list (12 seeded — the rest are on the store-locations page). **Reusable: the api_key + client_origin bypass likely unblocks the parked Nashville City Hive shops (Corkdorks, Frugal MacDoogal)**
