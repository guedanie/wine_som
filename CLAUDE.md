...
34. **Flavor-axis coverage gap** — surfaced by capability sweep #2 (item 33). `flavor_tags_for` derives tags only from a curated grape+region map (`flavor_profiles.py`); 28% of a 6k sample get ZERO tags (flavor-invisible). But that 28% is **three distinct causes**, and only a slice is map-fixable — don't over-scope to "recover 28%":
    ...
    - **(c) the genuinely map-fixable slice (~8–10%)**: a short list of real unmapped grapes (**Moscato ~118, Glera/Prosecco ~69, Nero d'Avola ~24**) + regions (**California ~109** — map has Napa/Sonoma but not the catch-all — **Veneto ~78, Sicily, Champagne, Loire, Alsace, Penedès**).
    
    Also: the parser clamps `flavors` to a **15-word vocab** (`FLAVOR_VOCAB`), so common asks (buttery, oaky, smoky, jammy, mineral, floral, citrus, tropical) are dropped before scoring, and the kw-hit fallback is dead (`tasting_notes` 100% empty, `flavor_profile` metadata dropped in item 33). **Compounding catch**: adding an aromatic grape like Moscato only helps if the vocab AND parser prompt gain matching words (floral/aromatic/sweet) — a map add without a vocab add is a no-op. **Scope when tackled**: (c)'s ~15 grape/region entries paired with the needed vocab words; leave (a)/(b) to enrichment + item 32. Conservative rule stands — an asserted flavor tag must be defensible, not guessed.
