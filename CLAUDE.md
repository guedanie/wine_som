...
10. ~~Mac mini enrichment server~~ ✅ **DONE 2026-07-09** — M4 Mac mini, 16 GB, repo at `~/dev/wine_app`. Vivino launchd (twice daily 10:00/16:00) + weekly extraction launchd (Sun 03:00 CT, chains `persist_structure.py`) both LIVE. Ollama runs as brew service (qwen2.5:7b + llama3.1:8b pulled). `sudo pmset -c sleep 0 disksleep 0` applied so schedules always fire (display can still sleep). Both wrappers pin `/usr/bin/python3` (Apple 3.9.6) and Slack-notify via shared `lib_notify_slack.sh`. **Next mini task**: mirror the Vivino wrapper for Twin Liquors (see #24) — the pattern is set
11. **Local LLM extraction cutover** — ⚙️ IN FLIGHT 2026-07-09 — `EXTRACTOR_BACKEND=ollama` wired into the weekly mini launchd; **initial backlog drain running** (5,516 → dropping; watcher fires `persist_structure.py` on completion). REMAINING: once drain completes + steady-state throughput is confirmed on the mini, drop Haiku from `weekly-scrape.yml` (still capped at 1500/run meanwhile as fallback)
...
24. ~~Twin Liquors scraper~~ ✅ **BUILT 2026-07-09; GitHub-blocked → mini 2026-07-10** — `backend/scrapers/twin_liquors.py`. Test run confirmed GitHub datacenter IPs are Cloudflare-1015-blocked (0/12 stores), so it runs on the residential-IP mini. Remaining: (a) add `run_twin_liquors_launchd.sh` + `com.somm.twin-liquors.plist` mirroring the Vivino ones (`lib_notify_slack.sh` already exists, so just source it) — pattern is set; (b) enumerate the full ~90-store merchant_id list (12 seeded). **Reusable: the api_key + client_origin bypass likely unblocks the parked Nashville City Hive shops (Corkdorks, Frugal MacDoogal)**
