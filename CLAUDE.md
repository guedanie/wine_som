...
31. **Name-directed full-inventory fallback** — when a prompt names a specific bottle and the targeted+breadth fetch (item 29) yields nothing, fall back to searching the entire zip-scoped (optionally type-filtered) inventory instead of the top-500 breadth sample.
...
33. ✅→⚙️ **Recommender country + fortified fixes** — a 78209 capability sweep (structural axes: type/region/country/grape/price/compound) found two gaps: (a) fortified wines unreachable via a typed request (enum has no `fortified`, only `dessert` — item 30 retyped Port/Sherry to fortified); (b) compound country+type queries surface on breadth-sample luck (fetch+scorer are country-blind; `white+Argentina`→0). Spec: `docs/superpowers/specs/2026-07-18-recommender-country-fortified-fixes-design.md` — fold fortified into `dessert` requests at the gate; match the intent place against region OR country in the targeted fetch + scorer. REMAINING: **second capability sweep over the softer axes — flavor/style, `avoid` (the one hard-exclusion path), and `body`/`_resolve_body`** (the first sweep didn't probe them; likeliest spot for the next hidden gaps).
