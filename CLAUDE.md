...
24. ~~Twin Liquors scraper~~ ✅ **DONE 2026-07-09** — `backend/scrapers/twin_liquors.py` + `run_twin_liquors_launchd.sh` + `com.somm.twin-liquors.plist` all live on the mini (Sun 04:00 CT weekly, smoke-tested via `launchctl start`). GitHub datacenter IPs are Cloudflare-1015-blocked (0/12 stores confirmed), which is why it lives on the residential-IP mini alongside Vivino. Remaining: enumerate the full ~90-store merchant_id list (12 seeded). **Reusable: the api_key + client_origin bypass likely unblocks the parked Nashville City Hive shops (Corkdorks, Frugal MacDoogal)**
