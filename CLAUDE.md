...
24. ~~Twin Liquors scraper~~ ✅ **BUILT 2026-07-09; GitHub-blocked → mini 2026-07-10** — `backend/scrapers/twin_liquors.py`. Test run confirmed GitHub datacenter IPs are Cloudflare-1015-blocked (0/12 stores), so it runs on the residential-IP mini (wrapper + runbook ready). Remaining: (a) schedule the mini launchd/cron weekly job; (b) enumerate the full ~90-store merchant_id list (12 seeded). **Reusable: the api_key + client_origin bypass likely unblocks the parked Nashville City Hive shops (Corkdorks, Frugal MacDoogal)**
