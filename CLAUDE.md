...
16. **Price intelligence** — ⚙️ design handoff: `frontend/design-system/handoffs/price-intelligence/`. Phase 1 done (`price_history` + delta trigger). **Phase A done 2026-07-13**: `PriceMarker` chip (4 variants) + dossier price-context module (drop/steady states, week-strip glyph, cheapest-row flag + struck was-price) fed by `utils/price_context.py` via `/api/wines/{id}`. REMAINING: B — chip on cards/picks + movement in recommend payload + somm voices drops; C — deals rail/screen; D — `price_watches` + sign-in nudge (notifier later). NOTE: restock variant needs a trigger extension (in_stock-only flips write no history row).
