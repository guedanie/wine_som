...
30. **NULL-`wine_type` backfill** — 5,443 wines (27.5%) have no `wine_type`; the type gate resolves them at query time (item 29), but a deterministic `infer_wine_type` backfill pass at rest is still owed, same shape as the grapes backfill (item 27).
